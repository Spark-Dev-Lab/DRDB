<template>
  <v-container fluid>
    ...
    <v-card class="ds-card mb-6" variant="flat">
      <v-toolbar color="transparent" density="compact">
        ...
        <div class="d-flex align-center bg-grey-lighten-4 rounded px-2 py-1">
          ...
          <v-btn
            ...
            density="comfortable"
            @click="nextPage"
            :disabled="page >= Families.length || page == 0"
          ></v-btn>
        </div>

        <div
          v-if="showFamilySearchPagination"
          class="d-flex align-center bg-blue-lighten-5 rounded px-2 py-1 ml-2"
        >
          <v-btn
            icon="mdi-page-first"
            variant="text"
            density="comfortable"
            @click="previousSearchResultsPage"
            :disabled="!hasPreviousSearchResultsPage"
          ></v-btn>
          <span class="text-body-2 font-weight-bold mx-3">
            Showing {{ currentSearchResultStart }}-{{ currentSearchResultEnd }} of
            {{ familySearchTotal }}
          </span>
          ...
          <v-btn
            ...
            variant="text"
            density="comfortable"
            @click="nextSearchResultsPage"
            :disabled="!hasNextSearchResultsPage"
          ></v-btn>
        </div>
      </v-toolbar>
      ...
      <div
        v-if="showFamilySearchPagination"
        class="px-4 pb-2 text-caption text-medium-emphasis"
      >
        Results are sorted by newest family first.
      </div>

      <v-alert
        v-if="showFamilySearchRefineHint"
        type="warning"
        variant="tonal"
        density="compact"
        border="start"
    ...
    <v-row class="mt-4 mx-n1" style="align-items: stretch">
      ...
      <v-col cols="12" md="5" class="d-flex px-1">
        ...
        >
          ...
          <v-card-text
            ...
            style="position: relative; z-index: 1; overflow-y: auto"
          >
            <v-row>
              <v-col cols="12" sm="8" class="text-center text-sm-left">
                  ...
                  <div class="text-center text-sm-left">
                    ...
                    >
                      ...
                      <v-btn
                        ...
                        size="x-small"
                        density="compact"
                        @click="copyToClipboard(String(currentFamily.id))"
                      ></v-btn>
                    </div>
                  </div>
                </div>
                ...
                <div
                  class="d-flex flex-wrap justify-center justify-sm-start gap-2 mt-4"
                  style="gap: 8px"
                >
                  <v-chip
                    size="small"
                    variant="tonal"
                    color="primary"
                    v-if="currentFamily.LanguagePrimary"
                  >
                    {{ currentFamily.LanguagePrimary }} ({{
                      currentFamily.EnglishPercent || 0
                    }}% EN)
                  </v-chip>
                  <v-chip
                    size="small"
                    variant="tonal"
                    color="secondary"
                    v-if="currentFamily.RacePrimary"
                  >
                  ...
                  >
                    ...
                    {{ currentFamily.Children.length === 1 ? "Child" : "Children" }}
                  </v-chip>
                </div>
              </v-col>
              ...
              >
                <div class="d-flex flex-column align-end gap-1" style="gap: 4px">
                  <v-chip size="x-small" color="primary" variant="flat"
                    >Total: {{ participationStats.Total }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('Completed', true)"
                    class="text-white"
                    variant="flat"
                    v-if="participationStats.Completed"
                    >Completed: {{ participationStats.Completed }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('Confirmed', false)"
                    variant="flat"
                    v-if="participationStats.Confirmed"
                    >Confirmed: {{ participationStats.Confirmed }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('No Show', false)"
                    class="text-white"
                    variant="flat"
                    v-if="participationStats['No Show']"
                    >No-Show: {{ participationStats["No Show"] }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('Cancelled', false)"
                    class="text-white"
                    variant="flat"
                    v-if="participationStats.Cancelled"
                    >Cancelled: {{ participationStats.Cancelled }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('Rejected', false)"
                    class="text-white"
                    variant="flat"
                    v-if="participationStats.Rejected"
                    >Rejected: {{ participationStats.Rejected }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('TBD', false)"
                    class="text-white"
                    variant="flat"
                    v-if="participationStats.TBD"
                    >TBD: {{ participationStats.TBD }}</v-chip
                  >
                  <v-chip
                    size="x-small"
                    :color="getTimelineColor('Rescheduling', false)"
                    class="text-white"
                    variant="flat"
                    v-if="participationStats.Rescheduling"
                    >Rescheduling: {{ participationStats.Rescheduling }}</v-chip
                  >
                </div>
              </v-col>
            </v-row>
            ...
            >
              <div class="d-flex align-center justify-space-between">
                <span class="text-body-2">
                  <strong
                    >{{ duplicateChildren.length }} possible duplicate
                    {{ duplicateChildren.length === 1 ? "child" : "children" }}</strong
                  >
                ...
                <v-btn
                  ...
                  @click="
                    childMergeCandidates = duplicateChildren;
                    childMergeDialog = true;
                    selectedMasterChildIds = duplicateChildren.map(() => null);
                  "
                >
            ...
            <div class="d-flex justify-space-between align-center mb-1 px-1">
              <span class="text-caption font-weight-bold text-uppercase text-muted"
                >Contact Info</span
              >
            ...
            <v-list density="compact" class="text-left px-0">
              <v-list-item prepend-icon="mdi-email-outline" class="px-0">
                <v-list-item-title class="d-flex align-center">
                  ...
                  <v-btn
                    v-if="currentFamily.Email"
                    icon="mdi-content-copy"
                    variant="text"
                    size="x-small"
                    density="compact"
                    class="ml-2"
                    @click="copyToClipboard(currentFamily.Email)"
                  ></v-btn>
                </v-list-item-title>
              </v-list-item>
              <v-list-item prepend-icon="mdi-phone-outline" class="px-0">
                <v-list-item-title class="d-flex align-center">
                  <span v-if="currentFamily.Phone">{{
                    PhoneFormated(currentFamily.Phone)
                  }}</span>
                  <span v-else class="text-muted">No phone</span>
                  <v-btn
                    v-if="currentFamily.Phone"
                    icon="mdi-content-copy"
                    variant="text"
                    size="x-small"
                    density="compact"
                    class="ml-2"
                    @click="copyToClipboard(currentFamily.Phone)"
                  ></v-btn>
                  <span v-if="currentFamily.CellPhone" class="text-muted ml-2"
                    >(Cell: {{ PhoneFormated(currentFamily.CellPhone) }})</span
                  >
                  <v-btn
                    v-if="currentFamily.CellPhone"
                    icon="mdi-content-copy"
                    variant="text"
                    size="x-small"
                    density="compact"
                    class="ml-2"
                    @click="copyToClipboard(currentFamily.CellPhone)"
                  ></v-btn>
                </v-list-item-title>
              </v-list-item>
              <v-list-item
                prepend-icon="mdi-map-marker-outline"
                :title="currentFamily.Address || 'No home address provided'"
                class="px-0"
              ></v-list-item>
            </v-list>
            ...
            <v-list density="compact" class="text-left px-0 pb-0">
              <v-list-item prepend-icon="mdi-calendar-clock" class="px-0">
                <v-list-item-title class="text-caption text-muted"
                  >Next Contact Date</v-list-item-title
                >
                <v-list-item-subtitle class="font-weight-medium">{{
                  currentFamily.NextContactDate
                    ? formatDate(currentFamily.NextContactDate)
                    : "Not set"
              ...
              <v-list-item
                ...
                class="px-0 pb-2"
                v-if="currentFamily.NextContactNote"
                style="background-color: transparent !important"
              >
                <v-list-item-title class="text-caption text-muted"
                  >Notes for next contact</v-list-item-title
                >
                <v-list-item-subtitle
      ...
      <v-dialog v-model="searchDialog" max-width="800px">
        <v-card class="ds-card" variant="flat">
          ...
          <v-card-text class="pt-6 pb-2">
            <div
              v-for="category in searchFamilyCategories"
          ...
          <v-card-actions class="px-6 pb-6 pt-0">
            <v-spacer></v-spacer>
            <v-btn color="error" variant="text" @click="searchDialog = false"
              >Cancel</v-btn
            >
            <v-btn
              color="primary"
          ...
          <v-card-text class="pt-2 pb-6 px-4" style="max-height: 70vh">
            <v-list density="compact" class="text-left">
              ...
              <v-list-item prepend-icon="mdi-account" class="px-2">
                <v-list-item-title class="text-caption text-muted"
                  >Primary Caregiver</v-list-item-title
                >
                <v-list-item-subtitle
                  class="font-weight-medium text-wrap mt-1"
                  style="line-height: 1.4; opacity: 1"
              ...
              <v-list-item prepend-icon="mdi-translate" class="px-2">
                ...
                >
                  ...
                  <span v-if="currentFamily.LanguageSecondary">
                    | Secondary: {{ currentFamily.LanguageSecondary }}</span
                  >
                  <span v-if="currentFamily.EnglishPercent !== null">
                    | English: {{ currentFamily.EnglishPercent }}%</span
                  >
                </v-list-item-subtitle>
              </v-list-item>
              ...
              <v-list-item prepend-icon="mdi-earth" class="px-2">
                ...
                >
                  ...
                  <span v-if="currentFamily.RaceSecondary">
                    | Secondary: {{ currentFamily.RaceSecondary }}</span
                  >
                </v-list-item-subtitle>
              </v-list-item>
              ...
              <v-list-item prepend-icon="mdi-car" class="px-2">
                <v-list-item-title class="text-caption text-muted"
                  >Vehicle Access</v-list-item-title
                >
                <v-list-item-subtitle
                  class="font-weight-medium text-wrap mt-1"
                  style="line-height: 1.4; opacity: 1"
              ...
              <v-list-item prepend-icon="mdi-brain" class="px-2">
                <v-list-item-title class="text-caption text-muted"
                  >Autism History</v-list-item-title
                >
                <v-list-item-subtitle
                  class="font-weight-medium text-wrap mt-1"
                  style="line-height: 1.4; opacity: 1"
                >
                  {{
                    currentFamily.AutismHistory === 1
                      ? "Yes"
                      : currentFamily.AutismHistory === 0
                      ? "No"
                      : "Unknown"
                  }}
                </v-list-item-subtitle>
      ...
      <v-col cols="12" md="4" class="d-flex px-1">
        <div v-if="currentFamily.id" style="width: 100%; overflow-y: auto; height: 500px">
          <ChildInfo
            ref="childInfo"
            ...
            @childAdded="updateFamilyAppointment"
          ></ChildInfo>
        </div>
        <v-card
          v-else
          class="ds-card d-flex flex-column align-center justify-center"
          variant="flat"
      ...
      <v-col cols="12" md="3" class="d-flex px-1">
        <div v-if="currentFamily.id" style="width: 100%; height: 500px">
          <NotesConversation
            class="h-100"
      ...
      <v-card class="ds-card" variant="flat">
        ...
        <v-card-actions class="px-6 pb-6 pt-0">
          ...
          <v-btn
            color="grey-darken-1"
            variant="text"
            @click="deleteFamilyDialog = false"
    ...
    <v-dialog v-model="deleteDialog" max-width="500px">
      <v-card class="ds-card" variant="flat">
        ...
        <v-card-text class="pt-6 pb-2">
          Are you sure you want to delete this schedule and its associated Google Calendar
        ...
        <v-card-actions class="px-6 pb-6 pt-0">
          ...
          <v-btn
            ...
            :disabled="isDeletingTimelineSchedule"
            >Cancel</v-btn
          >
          <v-btn
            color="error"
            variant="flat"
            :loading="isDeletingTimelineSchedule"
            @click="deleteTimelineSchedule"
            prepend-icon="mdi-delete"
            >Yes, Delete</v-btn
          >
        </v-card-actions>
        ...
        >
          ...
          >
            <v-tab value="timeline"
              ><v-icon start>mdi-timeline-text-outline</v-icon>Timeline</v-tab
            >
            <v-tab value="table"
              ><v-icon start>mdi-table</v-icon>Participation Record</v-tab
            >
          </v-tabs>

          <v-divider></v-divider>

          <v-window v-model="historyTab" style="flex: 1; overflow-y: auto">
            <v-window-item value="timeline" class="pa-6">
              <div
                v-if="reversedSchedules && reversedSchedules.length > 0"
              ...
              <div style="overflow-x: auto; padding-bottom: 24px; padding-top: 8px">
                <div
                  class="d-flex align-center flex-nowrap position-relative"
              ...
              >
                <v-icon size="large" class="mb-2" color="grey-lighten-1"
        ...
        <v-card-text class="pt-6 pb-2 bg-grey-lighten-4" style="max-height: 70vh">
          <div v-if="loadingDuplicates" class="text-center py-12">
            ...
            <div class="mt-4 text-muted">
              Scanning database for matching emails or phones...
            </div>
          </div>
          ...
          <v-expansion-panels v-else variant="accordion">
            ...
            >
              <v-expansion-panel-title class="font-weight-bold text-primary">
                {{ group.matchReason }}
                <v-chip size="x-small" color="error" class="ml-3"
                  >{{ group.families.length }} Records Found</v-chip
                >
              </v-expansion-panel-title>

              <v-expansion-panel-text class="bg-white pt-4">
                <v-alert type="info" variant="tonal" density="compact" class="mb-4">
                  Select the <strong>Master Record</strong> you want to keep. Study
                  histories will always be moved into the Master Record. Children can
                  optionally be merged too.
                </v-alert>

                <v-radio-group v-model="selectedPrimaryIds[index]">
                  <v-card
                    v-for="fam in group.families"
                    :key="fam.id"
                    variant="outlined"
                    class="mb-3 pa-3"
                    :style="
                      selectedPrimaryIds[index] === fam.id
                        ? 'border-color: var(--color-primary) !important; background-color: rgba(30, 64, 175, 0.05);'
                        : ''
                    "
                  >
                      ...
                      <div class="flex-grow-1">
                        <div class="d-flex justify-space-between align-center">
                          <strong class="text-subtitle-1">{{
                            fam.NamePrimary || "Unknown Parent"
                          }}</strong>
                        ...
                        >
                          ...
                          <v-chip
                            size="small"
                            :color="
                              fam.Children && fam.Children.length > 0 ? 'primary' : 'grey'
                            "
                            variant="tonal"
                            prepend-icon="mdi-human-child"
                          >
                            {{ fam.Children?.length || 0 }}
                            {{ fam.Children?.length === 1 ? "Child" : "Children" }}
                            <span
                              v-if="fam.Children && fam.Children.length > 0"
                              class="ml-1"
                            >
                              ({{
                          ...
                          <v-chip
                            size="small"
                            :color="
                              fam.Schedules && fam.Schedules.length > 0 ? 'teal' : 'grey'
                            "
                            variant="tonal"
                            prepend-icon="mdi-flask-outline"
                          >
                            {{ fam.Schedules?.length || 0 }}
                            {{ fam.Schedules?.length === 1 ? "Study" : "Studies" }}
                          </v-chip>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="d-flex align-center justify-space-between">
                    <div>
                      <div class="text-body-2 font-weight-bold">
                        Merge Children into Master Record?
                      </div>
                      <div class="text-caption text-muted mt-1">
                        <span v-if="mergeChildrenFlags[index]"
                          >Children from all records will be
                          <strong>combined</strong>
                          under the Master Record.</span
                        >
                        <span v-else
                          >Children will <strong>stay</strong> in their original family
                          records (only study histories are merged).</span
                        >
                      </div>
                    </div>
                    <v-switch
                ...
                <div class="d-flex justify-end mt-2">
                  <v-btn
                    ...
                    color="grey"
                    class="mr-2"
                    @click="dismissGroup(index)"
                    >Dismiss (Not Duplicates)</v-btn
                  >
                  <v-btn
                    color="warning"
                    variant="flat"
                    :disabled="!selectedPrimaryIds[index]"
                    @click="mergeGroup(index, group)"
                    >Merge & Clean</v-btn
                  >
                </div>
      ...
      <v-card class="ds-card" variant="flat">
        ...
        >
          <span
            class="text-h6 font-weight-bold"
            style="font-family: var(--ds-font-family-heading)"
            >Merge Duplicate Children</span
          >
          <v-btn
            icon="mdi-close"
            variant="text"
            density="comfortable"
            @click="childMergeDialog = false"
          ></v-btn>
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text class="pt-4">
          <v-alert type="info" variant="tonal" density="compact" class="mb-4">
            Select the <strong>Master Record</strong> to keep. The other record's
            appointments will be moved into it, then deleted.
          </v-alert>
          ...
          <div v-for="(pair, pi) in childMergeCandidates" :key="pi" class="mb-6">
            ...
            <v-radio-group v-model="selectedMasterChildIds[pi]">
              <v-card
                ...
                class="mb-2 pa-3"
                :style="
                  selectedMasterChildIds[pi] === c.id
                    ? 'border-color: var(--color-primary) !important; background: rgba(30,64,175,0.05)'
                    : ''
                "
              >
                <div class="d-flex align-center">
                  ...
                  <div class="ml-2">
                    ...
                    <div class="text-caption text-muted">
                      DoB: {{ c.DoB ? c.DoB.slice(0, 10) : "—" }} &nbsp;·&nbsp; Sex:
                      {{ c.Sex || "—" }} &nbsp;·&nbsp; ID: {{ c.id }}
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    async openDuplicateDialog() {
      ...
      try {
        ...
        this.$refs.confirmD.open("Error", "Failed to scan for duplicates.", {
          color: "error",
          noconfirm: true,
        });
      }
    ...
    dismissGroup(index) {
      ...
      this.duplicateGroups.forEach((_, i) => {
        ...
        newMergeFlags[i] =
          this.mergeChildrenFlags[oldIdx] !== undefined
            ? this.mergeChildrenFlags[oldIdx]
            : true;
      });
    ...
    async mergeGroup(index, group) {
      ...
      const secondaryIds = group.families
        .map((f) => f.id)
      ...
      const childNote = mergeChildren
        ? "Children from all records will be combined."
        : "Children will stay in their own records.";
      const confirmMsg = `Merge ${secondaryIds.length} record(s) into Master Record ${primaryLabel}?\n\n${childNote}\n\nThis cannot be undone.`;

      if (
        await this.$refs.confirmD.open("Confirm Merge", confirmMsg, { color: "warning" })
      ) {
        ...
        try {
          ...
          this.$refs.confirmD.open(
            "Success",
            "Records successfully merged and cleaned.",
          ...
          if (
            this.currentFamily.id === primaryId ||
            secondaryIds.includes(this.currentFamily.id)
          ) {
            this.searchFamily();
          }
        } catch (error) {
          console.error("Merge failed", error);
          this.$refs.confirmD.open("Error", "Failed to merge families.", {
            color: "error",
            noconfirm: true,
          });
        }
    ...
    async mergeChildPair() {
      ...
      const secondarySummary = this.childMergeCandidates.map((pair, pi) => {
        ...
        const masterName =
          [pair.a, pair.b].find((c) => c.id === masterId)?.Name || masterId;
        return { masterId, otherId, masterName };
      });

      const confirmLines = secondarySummary
        .map((s) => `Keep "${s.masterName}" (ID: ${s.masterId}), remove ID: ${s.otherId}`)
        .join("\n");
      ...
      try {
        ...
        this.$refs.confirmD.open("Success", "Children successfully merged.", {
          color: "success",
          noconfirm: true,
        });
        ...
        this.$refs.confirmD.open("Error", "Failed to merge children.", {
          color: "error",
          noconfirm: true,
        });
      }
    ...
    async loadFamilySearchPage(offset = 0) {
      ...
      if (families.length > 0) {
        ...
        this.$refs.confirmD.open(
          "No Results",
          Results.data.message || "No families found.",
    ...
    addFamily() {
      if (this.store.trainingMode) {
        this.$refs.confirmD.open(
          "Training Mode",
          "You are currently in Training mode.<br><br>Any family created under Training mode will only be accessible for training purpose.<br><br>If you want to create a record for a real family, please turn off the Training mode first.",
          { color: "warning", noconfirm: true }
        );
      }
      this.editedIndex = -1;
      ...
      try {
        ...
        if (error.response && error.response.data) {
          ...
          if (data.errors) {
            ...
            this.$refs.confirmD.open("Validation Failed", messages, {
              color: "error",
              noconfirm: true,
            });
          } else if (data.error) {
            this.$refs.confirmD.open("Error", data.error, {
              color: "error",
              noconfirm: true,
            });
          } else {
    ...
    async nextSearchResultsPage() {
      ...
      try {
        if (this.familySearchMode === "followup") {
          await this.loadFamilyFollowupPage(
            this.familySearchOffset + this.familySearchLimit
          );
        } else {
          await this.loadFamilySearchPage(
            this.familySearchOffset + this.familySearchLimit
          );
        }
      } catch (error) {
    ...
    async NoMoreContact() {
      ...
      if (
        ...
        try {
          ...
          this.$refs.confirmD.open(
            "Family Removed",
            "This family has been removed from the database.",
            { color: "warning", noconfirm: true }
          );
        } catch (error) {
          console.log(error);
          this.$refs.confirmD.open("Error", "Failed to update family.", {
            color: "error",
            noconfirm: true,
          });
        }
    ...
    async deleteTimelineSchedule() {
      ...
      try {
        ...
        if (this.currentFamily && this.currentFamily.Schedules) {
          this.currentFamily.Schedules = this.currentFamily.Schedules.filter(
            (s) => s.id !== this.scheduleToDelete.id
          );
          Object.assign(this.Families[this.page - 1], this.currentFamily);
        }
        ...
        this.$refs.confirmD.open(
          "Deleted",
          "Schedule and calendar event successfully deleted.",
          { color: "success", noconfirm: true }
        );
      } catch (error) {
        console.error(error);
        this.$refs.confirmD.open("Error", "Failed to delete the schedule.", {
          color: "error",
          noconfirm: true,
        });
      } finally {
    ...
    getTimelineColor(status, completed) {
      switch (status) {
        case "Completed":
          return "#01579B";
        case "Confirmed":
          return completed ? "#01579B" : "light-blue-accent-2";
        case "TBD":
  ...
  computed: {
    ...
    hasNextSearchResultsPage() {
      return (
        this.showFamilySearchPagination &&
        this.currentSearchResultEnd < this.familySearchTotal
      );
    },
    reversedSchedules() {
    ...
    duplicateChildren() {
      ...
      for (let i = 0; i < kids.length; i++) {
        for (let j = i + 1; j < kids.length; j++) {
          ...
          const sameDoB = a.DoB && b.DoB && a.DoB.slice(0, 10) === b.DoB.slice(0, 10);
          if (sameName || sameDoB) {
            pairs.push({ a, b });
            seen.add(key);
          }
